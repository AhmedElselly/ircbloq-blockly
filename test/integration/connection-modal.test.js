...
// The tests below require IrcBloq Link to be unavailable, so we can trigger
...
describe('Hardware extension connection modal', () => {
    ...
    test('Message saying IrcBloq Link is unavailable (BLE)', async () => {
        await driver.quit();
        driver = getDriver();
        ...
        findByText('IrcBloq Link'); // IrcBloq Link is mentioned in the error modal

        const logs = await getLogs();
        await expect(logs).toEqual([]);
    });

    test('Message saying IrcBloq Link is unavailable (BT)', async () => {
        await loadUri(uri);
